  ...
  class ThreeDExtension {
    ...
    addPhysicsToObject(args) {
        ...
        if (shapeType === 'box') {
            ...
            // 角丸箱型：縁石の角に引っかかる問題を解消するため角を丸める
            // borderRadius は短辺の 10% を目安にする（大きすぎると形状が変わりすぎる）
            const borderRadius = Math.min(size.x, size.y, size.z) * 0.10;
        ...
        this.physicsBodies.set(id, {
            ...
            _roundRadiusRatio: (shapeType === 'roundedBox') ? 0.10 : 0
        });
    ...
    getColliderOffset(args) {
        ...
        if (shapeType === RAPIER.ShapeType.Cuboid) {
            ...
            const borderRadius = Math.min(x, y, z) * 0.10;
    ...
    setRoundRadius(args) {
        ...
        // 現在の halfExtents + 既存 borderRadius から元の外形サイズを復元
        const he = oldCollider.halfExtents(); // inner half extents
        const oldRatio = physicsData._roundRadiusRatio || 0.10;
        // outer half = he + oldBorderRadius = he + min(outerSize)*oldRatio
        // outerSize = he*2 + oldBorderRadius*2 → borderRadius = min(outerSize)*ratio
        // 簡易復元: outerHalf = he / (1 - oldRatio)
        const outerHalfX = he.x / (1 - oldRatio);
        const outerHalfY = he.y / (1 - oldRatio);
        const outerHalfZ = he.z / (1 - oldRatio);

        const borderRadius = Math.min(outerHalfX, outerHalfY, outerHalfZ) * 2 * ratio;
        const innerHalfX = outerHalfX - borderRadius;
        const innerHalfY = outerHalfY - borderRadius;
        const innerHalfZ = outerHalfZ - borderRadius;
    ...
    updateSizeAndPosition() {
      ...
      if (this.stageCanvas) {
        ...
        [this.replacementCanvas, this.luminanceMaskCanvas, this.threeMaskedCanvas].forEach(c => {
          if (c) {
    ...
    _updateAllCars(dt) {
      ...
      for (const [id, car] of this.carControllers) {
        ...
        // 速度が遅い時はステアリング無効
        if (Math.abs(car.speed) < 0.005) car.steering = 0;
        ...
        const vx = forward.x * car.speed * car.direction;
        const vz = forward.z * car.speed * car.direction;

        if (physicsData) {
          const currentVel = physicsData.rigidBody.linvel();
          physicsData.rigidBody.setLinvel({ x: vx, y: currentVel.y, z: vz }, true);

          // 向きをRapierに同期
          physicsData.rigidBody.setRotation(
            { x: obj.quaternion.x, y: obj.quaternion.y,
              z: obj.quaternion.z, w: obj.quaternion.w }, false
          );
          physicsData.rigidBody.setAngvel({ x: 0, y: 0, z: 0 }, false);
